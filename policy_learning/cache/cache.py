...
class CacheSet(object):
  ...
  def __init__(self, set_id, num_cache_lines, eviction_policy,
    ...
    self._access_times1 = {}  #LIP
    self._access_count = {}  # maps the hit number of the Cache line 
    self.crf={}  #LRFU
    self.last={}  #LRFU
    self.rrpv={}  #RRPV
  ...
  def read(self, pc, address, observers=None):
    ...
    def update_cache_set(cache_access):
      ...
      line_to_evict, cache_line_scores = self._eviction_policy(
          cache_access, self._access_times, self._access_times1, self._access_count, self.crf, self.rrpv)
      
      self._read_counter += 1
      self._access_times[address] = self._read_counter
      self._access_times1[address] = self._read_counter
      r = random.random()

      if address in self._cache_lines:
        # update the PC associated with the cache line
        self.crf[address] = 1 + ((0.5)**(0.7*(self._access_times[address] - self.last[address]))) * self.crf[address]
        self._cache_lines[address] = pc
        self._access_count[address] += 1
        self.last[address] = self._read_counter
        self.rrpv[address] = 0
        flag = False
        for i in self.rrpv.values():
          if i == 3:
            flag = True
            break
        if flag == False:
          for k,v in self.rrpv.items():
            if k != address:
              self.rrpv[k] = v + 1 
        return True, EvictionDecision(False, cache_line_scores)

      if r > 0.03125:
        self._access_times1[address] = 0
      self.rrpv[address] = 2
      flag = False
      for i in self.rrpv.values():
          if i == 3:
      ...
      if evict:
        self._access_count[line_to_evict] = 0
...
class Cache(object):
  ...
  def eviction_change(config):
    def scorer_from_config1(config):
      scorer_type = config.get("type")
      if scorer_type == "lru":
        return eviction_policy_mod.LRUScorer()
      elif scorer_type == "random":
        return 
      elif scorer_type == "mru":
        return eviction_policy_mod.LRUScorer()
      elif scorer_type == "lfu":
        return eviction_policy_mod.LFUScorer()
      elif scorer_type == "bip":
        return eviction_policy_mod.BIPScorer()
      elif scorer_type == "lrfu":
        return eviction_policy_mod.LRFUScorer()
      elif scorer_type == "srrip":
        return eviction_policy_mod.SRRIPScorer()
      else:
        raise ValueError("Invalid scorer type: {}".format(scorer_type))

    policy_type = config.get("policy_type")
    if policy_type == "greedy":
      scorer = scorer_from_config1(config.get("scorer"))
      return eviction_policy_mod.GreedyEvictionPolicy(
          scorer, config.get("n", 0))
    elif policy_type == "greedy1":
      scorer = scorer_from_config1(config.get("scorer"))
      return eviction_policy_mod.MRUEvictionPolicy(
          scorer, config.get("n", 0))
...
class BernoulliProcessStatistic(object):
  ...
  @property
  def num_trials1(self):
    return self._trials1
  ...
  @property  #save recent successes
  def num_successes1(self):
    return self._successes1
  ...
  def success1_rate(self):
    if self.num_trials1 == 0:
      raise ValueError("Success rate is undefined when num_trials is 0.")
    return self.num_successes1 / self.num_trials1
