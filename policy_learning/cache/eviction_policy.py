...
class EvictionPolicy(six.with_metaclass(abc.ABCMeta, object)):
  ...
  @abc.abstractmethod
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
...
class GreedyEvictionPolicy(EvictionPolicy):
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    scores = self._cache_line_scorer(cache_access, access_times, access_times1, access_count, crf1, rrpv1)
...
class MRUEvictionPolicy(EvictionPolicy):
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    scores = self._cache_line_scorer(cache_access, access_times, access_times1, access_count, crf1, rrpv1)
    sorted_cache_lines = heapq.nlargest(
        self._n + 1, scores.keys(), key=lambda line: scores[line])
    to_evict = sorted_cache_lines[-1] if sorted_cache_lines else None
    return to_evict, scores
...
class MixturePolicy(EvictionPolicy):
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    ...
    line_to_evict, scores = policy(cache_access, access_times, access_times1, access_count, crf1, rrpv1)
    if self._scoring_policy_index is not None:
      scoring_policy = self._policies[self._scoring_policy_index]
      _, scores = scoring_policy(cache_access, access_times, access_times1, access_count, crf1, rrpv1)
...
class BIPScorer(CacheLineScorer):
  """Returns scores equal to how recently each line was used.
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    scores = {line: access_times1[line]
              for (line, _) in cache_access.cache_lines}
    return scores

class LFUScorer(CacheLineScorer):
  """Returns scores equal to how recently each line was used.
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    scores = {line: access_count[line]
              for (line, _) in cache_access.cache_lines}
    return scores

class LRFUScorer(CacheLineScorer):
  """Returns scores equal to how recently each line was used.
  ...
  def __call__(self, cache_access, access_times, access_times1, access_count, crf1, rrpv1):
    scores = {line: crf1[line]
              for (line, _) in cache_access.cache_lines}
    return scores

class SRRIPScorer(CacheLineScorer):
  """Returns scores equal to how recently each line was used.
